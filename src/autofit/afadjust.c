...
#define FT_COMPONENT  afadjust
  ...
  */
                  ...
                  FT_Memory  memory )
  {
    ...
    if ( value & 1 )
    {
      FT_UInt  substitute = (FT_UInt16)value >> 1;


      /*
        OpenType features like 'unic' map lowercase letter glyphs to
        uppercase forms (and vice versa), which could lead to the use of
        wrong entries in the adjustment database.  For this reason we don't
        overwrite, prioritizing cmap entries.
        ...
        lowercase and uppercase characters in the adjustment database that
        have diacritics on different vertical sides (for example, U+0122 'Ģ'
        and U+0123 'ģ').  The auto-hinter could then perform a topological
      ...
      */
      ...
      if ( reverse_map->used > used )
      {
        ...
        if ( subst )
        {
          error = add_substitute( *subst, codepoint,
    ...
    {
      FT_UInt16*  substitutes = (FT_UInt16*)value;
      FT_UInt16   count       = substitutes[0];
      FT_UInt16   n;


      for ( n = 1; n <= count; n++ )
      {
        FT_UInt  substitute = substitutes[n];
                                ...
                                AF_StyleMetrics  metrics )
  {
    ...
    *map = NULL;
...

    {
      ...
      */
      ...
      while ( ft_hash_num_iterator( &hash_idx,
                                    ...
                                    subst_map ) )
      {
        size_t*  val = ft_hash_num_lookup( glyph_idx, *map );
        ...
        error = add_substitute( value, codepoint, *map, subst_map, memory );
        if ( error )
          break;
      }
    ...
    Exit_HarfBuzz:
      ...
      hash_idx = 0;
      while ( ft_hash_num_iterator( &hash_idx, NULL, &value, subst_map ) )
      {
        if ( !( value & 1 ) )
        {
          FT_UInt16*  substitutes = (FT_UInt16*)value;
